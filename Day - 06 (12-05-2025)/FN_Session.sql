...
COMMIT;
-- After a COMMIT, the transaction ends and all savepoints are deleted, so ROLLBACK TO SAVEPOINT is no longer possible.
-- ROLLBACK TO after_alice;
SELECT * FROM tbl_bank_accounts;
...
-- Values in existing rows change => Non repeatble	
-- New rows appear or disappear => phantom
